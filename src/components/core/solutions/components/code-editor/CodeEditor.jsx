...
const CodeEditor = ({ solution, lang, setSolution, setLang, handleCodeChange }) => {
  ...
  // Sync external solution to internal codeMap
  useEffect(() => {
    if (solution && lang) {
      setCodeMap((prev) => ({
        ...prev,
  ...
  const handleLanguageChange = (e) => {
    ...
    setLang(newLang); // Use the value directly, do not lowercase
    handleCodeChange(newLang); // fetch solution if it exists
    setOutput("");
  ...
  const runCode = useCallback(() => {
    ...
    if (lang.toLowerCase() === "javascript") {
      ...
      setOutput(
        getUnsupportedLanguageMessage(lang) + code
      );
      setShowConsole(true);
      setEditorFlex(0.6);
      return;
  ...
  return (
    <div ref={containerRef} className={styles.playgroundContainer}>
      <div className={styles.playgroundToolbar}>
        <select value={lang} onChange={handleLanguageChange} className={styles.playgroundSelect}>
      ...
      <div className={styles.playgroundMain}>
        <div style={{ flex: editorFlex, minHeight: "100px" }}>
          <MonacoEditor
            key={lang}
